...
router.get("/", async (req, res) => {
  ...
  const stavbaId =
    rawStavbaId !== undefined ? parseInt(rawStavbaId as string) : undefined;
  ...
  if (stavbaId !== undefined && !isNaN(stavbaId)) {
    connectionsQuery = connectionsQuery.where(
      eq(connectionsTable.stavbaId, stavbaId)
    );
  }
  ...
  const allItems =
    connectionIds.length > 0
      ? await db
          ...
          .innerJoin(
            materialsTable,
            eq(connectionItemsTable.materialId, materialsTable.id)
          )
