...
import { db } from "../db";
import { stavbyTable } from "../schema";
...
router.get("/", async (_req, res) => {
  const stavby = await db
    .select()
    .from(stavbyTable)
    .orderBy(asc(stavbyTable.order), asc(stavbyTable.createdAt));
  res.json(stavby);
});
