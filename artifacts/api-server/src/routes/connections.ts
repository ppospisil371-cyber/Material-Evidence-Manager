...
router.post("/:id/copy", async (req, res) => {
  ...
  const [source] = await db.select().from(connectionsTable).where(eq(connectionsTable.id, id));
...
router.put("/:id/items", async (req, res) => {
  ...
  await db.delete(connectionItemsTable).where(eq(connectionItemsTable.connectionId, id));
